...
Commutative encoder pretraining uses a fixed masked-probe ontology rather than full-volume reconstruction. Probe targets are derived from a shared coarse spatiotemporal tensor, self-probes are trained against input-derived probe targets, cross-probes learn from detached opposite-branch self-probe predictions, and branch agreement is encouraged through scheduled prototype alignment rather than direct latent MSE by default. The probe types are:
