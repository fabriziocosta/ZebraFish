...
function formatGpuHours(value: number | null | undefined): string {
  return value === null || value === undefined || !Number.isFinite(value) ? "not available" : `${formatNumber(value)} GPU h`;
}
...
export function InvestigationHeader({ data }: { data: Investigation }) {
  ...
  return <header className="mission-header">
    <div><div className="eyebrow">COMMUTATIVE REPRESENTATION / EXPERIMENT CONTROL</div><h1>Commutative Representation : Zebrafish experiment</h1></div>
...
export function MetaControllerCard({ data, onControl, onRunNow }: { data: Investigation; onControl: (controller: "meta" | "campaign", action: "start" | "stop" | "continue") => void; onRunNow: () => void }) {
  ...
  return <Card title="Meta-controller" eyebrow="SYSTEM SUPERVISION" className="meta-controller-card">
    ...
    <div className="meta-controller-footer"><span>Campaign controls affect execution only; scientific candidates remain autonomous.</span><div><button onClick={() => onControl("campaign", "start")}>start campaign</button><button onClick={() => onControl("campaign", "continue")}>continue campaign</button><button className="danger-button" onClick={() => onControl("campaign", "stop")}>stop campaign</button></div></div>
  </Card>;
}
  ...
  return <Card title="Domain guidance" eyebrow="BIOLOGICAL AND IDENTIFIABILITY TESTS" className="domain-guidance-card">
    ...
    <div className="domain-constraint-list">
      {guidance.constraints.map((constraint) => <section className={`domain-constraint domain-${constraint.status}`} key={constraint.id}>
        <button className="domain-constraint-heading" onClick={() => onSelect(constraint.title, constraint)}><span><Badge tone="neutral">{constraint.role.replaceAll("_", " ")}</Badge><strong>{constraint.title}</strong><small>{constraint.labels.join(" ↔ ")} · {constraint.support.sufficient === false ? "independent-unit support insufficient" : constraint.support.sufficient === true ? "independent-unit support available" : "independent-unit support not evaluated"}</small></span><Badge tone={toneFor(constraint.status)}>{constraint.status.replaceAll("_", " ")}</Badge></button>
        <div className="domain-checks">{constraint.checks.map((check) => {
          ...
          return <button key={`${constraint.id}-${check.metric}`} onClick={() => onSelect(check.metric.replaceAll("_", " "), check)}>
            <span className="domain-metric-name">{check.metric.replaceAll("_", " ")}</span>
            <strong>{formatNumber(check.value)}</strong>
            <span>95% CI {interval ? `${formatNumber(interval[0])}–${formatNumber(interval[1])}` : "unavailable"}</span>
            <span>baseline {formatNumber(typeof baseline?.mean === "number" ? baseline.mean : null)}</span>
            <span>Δ {formatNumber(check.delta)}</span>
            <Badge tone={toneFor(check.status)}>{check.status.replaceAll("_", " ")}</Badge>
          </button>;
        })}</div>
      </section>)}
    </div>
