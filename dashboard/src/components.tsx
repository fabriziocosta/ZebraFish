...
export function DecisionQueue({ data, onSelect }: { data: Investigation; onSelect: (candidate: Candidate) => void }) {
  ...
  return <Card title="Next decisions" eyebrow="AUTONOMOUS POLICY QUEUE">
    {preferred && <section className="automatic-next"><div className="decision-heading"><span className="eyebrow">NEXT AUTOMATIC DECISION</span><span className="decision-badge decision-badge-automatic">Automatic next</span></div><strong>{formatText(preferred.title)}</strong><p>{formatText(preferred.rationale || preferred.purpose || "No rationale recorded.")}</p><span className="muted">value/GPU hour {formatNumber(preferred.value_per_gpu_hour)} · information gain {formatNumber(preferred.expected_information_gain)} · scientific value {formatNumber(preferred.scientific_value)}</span></section>}
