...
export function MetaControllerCard({ data, onControl }: { data: Investigation; onControl: (controller: "meta" | "campaign", action: "start" | "stop" | "continue") => void }) {
  const meta = data.meta_controller;
  const severityTone = meta.severity === "critical" || meta.status === "meta_controller_safe_stop" ? "critical" : meta.severity === "warning" || meta.status === "verification_failed" ? "warning" : "healthy";
  ...
  return <Card title="Meta-controller" eyebrow="SYSTEM SUPERVISION" className="meta-controller-card">
    <div className="meta-controller-heading"><div><Badge tone={severityTone}>{meta.status.replaceAll("_", " ")}</Badge><p className="meta-summary">{formatText(meta.summary)}</p></div><div className="controller-buttons"><span className="muted">Operational controls</span><div><button onClick={() => onControl("meta", "start")}>start</button><button onClick={() => onControl("meta", "continue")}>continue</button><button className="danger-button" onClick={() => onControl("meta", "stop")}>stop</button></div></div></div>
    <div className="meta-controller-grid"><div><span className="eyebrow">LAST RUN</span><strong>{meta.last_run_at ? new Date(meta.last_run_at).toLocaleString() : "—"}</strong></div><div><span className="eyebrow">NEXT RUN</span><strong>{meta.next_run_at ? new Date(meta.next_run_at).toLocaleString() : "—"}</strong></div><div><span className="eyebrow">MANDATE</span><strong>{meta.mandate_version ? meta.mandate_version.slice(0, 20) : "—"}</strong></div><div><span className="eyebrow">ROLLBACK</span><strong>{meta.rollback_available ? "available" : "not available"}</strong></div></div>
